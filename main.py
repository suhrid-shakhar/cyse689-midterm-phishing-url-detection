...
X_train, X_test, y_train, y_test, feature_names = load_and_preprocess_data(test_size=0.6)  

# print("\nRunning Logistic Regression")
# model, y_pred = logistic_regression.logistic_regression_model(X_train, y_train, X_test)
# plot_learning_curve(model, X_train, y_train, title='Logistic Regression', cv=20)
...
print("\nRunning XGBoost")
model, y_pred = xgboost.xgboost_model(X_train, y_train, X_test)
plot_learning_curve(model, X_train, y_train, title='XGBoost', cv=20)
